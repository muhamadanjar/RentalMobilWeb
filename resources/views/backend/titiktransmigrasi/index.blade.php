        ...
        <div class="col-md-12">
            ...
            <div class="panel-body">
                <table class="table table-striped table-bordered table-hover" id="table_dom">
                    ...
                    <tbody>
                    @foreach($titik as $migrasi)
                    <tr>
                        <td>{{ $migrasi->name }}</td>
                        <td>{{ $migrasi->x }}</td>
                        <td><span class="badge">{{ $migrasi->y }}</span></td>
                        <td class="text-center">
                            <form class="form-delete" method="post" action="{{ route('backend.titiktransmigrasi.destroy', [$migrasi->id]) }}">
                            <input name="_method" type="hidden" value="DELETE">
                            {{ csrf_field()}}
                            <div class="btn-group text-center">
                                <a href="{{ route('backend.titiktransmigrasi.edit', [$migrasi->id]) }}" class="btn btn-default btn-sm">Edit</a>
