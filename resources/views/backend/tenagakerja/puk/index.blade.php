        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <div class="panel-toolbar text-right">
                        <div class="btn-group">
                            <a href="{{ route('backend.tenagakerja.puk.create') }}" class="btn btn-primary btn-sm btn-flat"> Tambah</a> 
                        </div>
                    </div>
                </div>
            <div class="panel-body">
                <table class="table table-striped table-bordered table-hover" id="table_dom">
                    ...
                    <tbody>
                    @foreach($puk as $migrasi)
                    <tr>
                        <td>{{ $migrasi->kegiatan }}</td>
                        <td>{{ $migrasi->tahun }}</td>
                        <td><span class="badge">{{ $migrasi->jumlah }}</span></td>
                        <td class="text-center">
                            <form class="form-delete" method="post" action="{{ route('backend.tenagakerja.puk.destroy', [$migrasi->id]) }}">
                            <input name="_method" type="hidden" value="DELETE">
                            {{ csrf_field()}}
                            <div class="btn-group text-center">
                                <a href="{{ route('backend.tenagakerja.puk.edit', [$migrasi->id]) }}" class="btn btn-default btn-sm">Edit</a>
